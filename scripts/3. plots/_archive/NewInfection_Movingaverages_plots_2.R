...
rki_r <- max(sort(list.files("rnull/")))
# Load this file
rki_r <- read.csv2(paste0("rnull/",rki_r))
rki_r$X <- NULL # Exclude this count variable indicating lines
rki_r <- select(rki_r, Datum, R, lb_R, ub_R)
colnames(rki_r) <- c("date","R_rki","R_lb_rki","R_ub_rki")
rki_r$date <- as.Date(rki_r$date, format="%Y-%m-%d")
...
rki_data_ma$r_error <- ifelse(rki_data_ma$cases == 0, 1, 0)
rki_data_ma$r_7error <- NA
...
for(i in 2:nrow(rki_data_ma)){
  if(rki_data_ma$state[i] == rki_data_ma$state[i-1]){
    ...
    if(rki_data_ma$day[i] >= windowr0){
      rki_data_ma$r_7error[i] <- ifelse(sum(rki_data_ma$r_error[(i-(windowr0-1)):i]) >= 1, 1, 0)
    }
    
    if(rki_data_ma$day[i] >= 7){
      rki_data_ma$mainf[i] <- (sum(rki_data_ma$cases[(i-6):i])/7)
      rki_data_ma$mainf_sd[i] <- sd(rki_data_ma$cases[(i-6):i])
      #rki_data_ma$r_7error[i] <- ifelse(rki_data_ma$mainf[i] == 0, 1, 0)
...
rki_county_ma$r_error <- ifelse(rki_county_ma$cases == 0, 1, 0)
rki_county_ma$r_7error <- NA
...
for(i in 2:nrow(rki_county_ma)){
  if(rki_county_ma$nuts[i] == rki_county_ma$nuts[i-1]){
    ...
    if(rki_county_ma$day[i] >= windowr0){
      rki_county_ma$r_7error[i] <- ifelse(sum(rki_county_ma$r_error[(i-(windowr0-1)):i]) >= 1, 1, 0)
    }
...
# Create state data #
#-------------------#
states <- c(unique(rki_data_ma$state))
counties <- c(unique(as.character(rki_county_ma$nuts)))
startdate <- as.Date("2020-03-01", "%Y-%m-%d")

for(i in 1:length(states)){
  # select data
  tempstate <- subset(rki_data_ma, rki_data_ma$state %in% states[i]) # subset to state i
  statestart <- min(tempstate$reportdate) # select minimum date
  #if(statestart < enddate){
  # end #
  
  # create state time data
  statetime <- as.data.frame(seq(statestart, enddate, by = 1)) # statetime data
  ...
  tstart <- tstart[1:(length(tstart)-cutoff)]
  tend  <- tstart + (windowr0 - 1)
  tenddate <- tempstate$reportdate[tend[length(tend)]]
  # end #
  ...
  output$date <- tempstate$reportdate[(windowr0+1):which(grepl(tenddate,tempstate$reportdate))]
  output$name <- states[i]
  output <- merge(output, rki_data_ma, by.x = c("name","date"), by.y = c("state","reportdate"), all.x = TRUE)
  output <- dplyr::select(output, name, date, t_start, t_end, "Mean(R)", "Quantile.0.025(R)","Quantile.0.975(R)", cases, cumcases, mainf, mainf_sd, Einwohnerzahl, infectper, cuminfectper, mainfper, mainfper_sd, r_7error)
  output[,"Mean(R)"] <- ifelse(output$r_7error == 1, NA, output[,"Mean(R)"])
  output[,"Quantile.0.025(R)"] <- ifelse(output$r_7error == 1, NA, output[,"Quantile.0.025(R)"])
  output[,"Quantile.0.975(R)"] <- ifelse(output$r_7error == 1, NA, output[,"Quantile.0.975(R)"])
  if(states[i] == "federal"){
    output <- merge(output, rki_r, by = "date", all.x = TRUE)
  } 
...
for(i in 1:length(counties)){
  ...
  tstart <- tstart[1:(length(tstart)-cutoff)]
  tend  <- tstart + (windowr0 - 1)
  tenddate <- tempcounty$reportdate[tend[length(tend)]]
...
# first filter for counties in state (rki_statenuts)
  ...
  for(j in 1:length(tempcounties)){
    ...
    countydata <- mget(ls(envir=.GlobalEnv, pattern=paste0("dat_",as.character(tempcounties[j]))))[[1]]
      ...
      countydata <- mget(ls(envir=.GlobalEnv, pattern=paste0("dat_berlin")))[[1]]
    }
    ...
    ger_rnull <- ger[(nrow(ger)-30):nrow(ger),]
    state_rnull <- state[(nrow(state)-30):nrow(state),]
    countydata_rnull <- countydata[nrow(countydata)-30:nrow(countydata),]
    
    
    
    # let last digit be dividable by 0.2 without adding digits
    ylimvalues <- round(c(-0.5,(max(c(max(ger_rnull$Quantile.0.975, na.rm = T)
                                   ,max(state_rnull$Quantile.0.975, na.rm = T)
                                   ,max(countydata_rnull$Quantile.0.975, na.rm = T),1)
    ))))
    yaxpvalues <- c(0,round((max(c(max(ger_rnull$Quantile.0.975, na.rm = T)
                                   ,max(state_rnull$Quantile.0.975, na.rm = T)
                                   ,max(countydata_rnull$Quantile.0.975, na.rm = T)
    ))),1), round(round((max(c(max(ger_rnull$Quantile.0.975ger_rnull$Quantile.0.975, na.rm = T)
                               ,max(state_rnull$Quantile.0.975, na.rm = T)
                               ,max(countydata_rnull$Quantile.0.975, na.rm = T)
    ))),1) / 0.2))
    ...
    # seq(1, nrow(ger_rnull), 1)
    # seq(nrow(ger_rnull) - 30, nrow(ger_rnull), 1)
    
    png(paste0("../../outcome/R0plots/",tempcounties[j],"_rnull.png"), width=10, height=5, bg = NA, units = 'in', res = 300)
    par(mar=c(7.5, 5, 1, 0)) 
    plot(seq(1, nrow(ger_rnull), 1)
         , las = 1
         , type="n"
         , ylim=ylimvalues
         , xlim=c(nrow(ger_rnull)-30,nrow(ger_rnull))
         , yaxp=c(-0.3, yaxpvalues[2], yaxpvalues[3])
         , xlab=""
         , xaxt="n" 
         , yaxt="n"
         , ylab="Durschnittliche Reproduktionszahl [R0]"
         , main= "Gleitende durchschnittliche Reproduktionszahl von COVID-19 (über 4 Tage)"
         , cex.lab = .9
         , cex.main = .9
         , col.axis = "darkgray"
         , col.lab = "darkgray" 
    ...
    # R0 = 1 line
    segments(0, 1, nrow(ger_rnull), 1, col = "gray", lty = 2, lwd = 2)
    
    # state_rnull plot
    for(n in 1:nrow(state_rnull)){
      segments((state_rnull$t_start[n]-1) + nrow(ger_rnull) - nrow(state_rnull) + 0.2, state_rnull$Quantile.0.025[n], (state_rnull$t_start[n]-1) + nrow(ger_rnull) - nrow(state_rnull)+0.2, state_rnull$Quantile.0.975[n], lty = 2, lwd = 1.75, lend = 0, col = adjustcolor( "red", alpha.f = .5))
    }
    #polygon(c(((nrow(ger_rnull) - nrow(state_rnull)) + seq(1, nrow(state_rnull), 1))
    #          ,rev(((nrow(ger_rnull) - nrow(state_rnull)) + seq(1, nrow(state_rnull), 1))))
    #        ,c(state_rnull$Quantile.0.975,rev(state_rnull$Quantile.0.025))
    #        ,col= adjustcolor( "darkgray", alpha.f = .2), border = adjustcolor( "darkgray", alpha.f = .2), cex = 1.75)  
    points((state_rnull$t_start-1 + nrow(ger_rnull) - nrow(state_rnull) + 0.2), state_rnull$Mean, pch=20, cex=1, col=adjustcolor( "red", alpha.f = .8))   
    
    # federal plot
    for(n in 1:nrow(ger_rnull)){
      segments(er_rnull$t_start-min(ger_rnull$t_start) - 0.2, ger_rnull$Quantile.0.025[n], ger_rnull$t_start-min(ger_rnull$t_start) - 0.2, ger_rnull$Quantile.0.975[n], lty = 1, lwd = 1.75, lend = 0, col = adjustcolor( "purple", alpha.f = .4))
    }
    #polygon(c(seq(1, nrow(ger_rnull), 1),rev(seq(1, nrow(ger_rnull), 1)))
    #        ,c(ger_rnull$Quantile.0.975,rev(ger_rnull$Quantile.0.025))
    #        ,col=adjustcolor( "darkgray", alpha.f = .4), border = adjustcolor( "darkgray", alpha.f = .4), cex = 1.75)  
    points(ger_rnull$t_start-min(ger_rnull$t_start) - 0.2, ger_rnull$Mean, pch=20, cex=1, col=adjustcolor( "purple", alpha.f = .8))   
    
    # county plot
    #higherq <- ifelse(is.na(countydata_rnull$Quantile.0.975) == TRUE, 0, countydata_rnull$Quantile.0.975)
    #lowerq <- countydata_rnull$Quantile.0.025
    #minq <- min(which(grepl("^\\d+", lowerq)))
    #maxq <- max(which(grepl("^\\d+", lowerq)))
    #for(k in 1:length(lowerq)){
    #  l <- minq-1
    #  lowerq[(k)] <- ifelse(is.na(lowerq[k]) == TRUE, 0, na.omit(countydata_rnull$Quantile.0.025)[k-l])
    #}
    for(n in 1:nrow(countydata_rnull)){
      segments((countydata_rnull$t_start[n]-1) + nrow(ger_rnull) - nrow(countydata_rnull), countydata_rnull$Quantile.0.025[n], (countydata_rnull$t_start[n]-1) + nrow(ger_rnull) - nrow(countydata_rnull), countydata_rnull$Quantile.0.975[n], lty = 2, lwd = 1.75, lend = 0, col = adjustcolor( "black", alpha.f = .4))
    }
    #polygon(c(((nrow(ger_rnull) - nrow(countydata_rnull)) + seq(1, nrow(countydata_rnull), 1))
    #        ,rev(((nrow(ger_rnull) - nrow(countydata_rnull)) + seq(1, nrow(countydata_rnull), 1))))
    #        ,c(higherq,rev(lowerq))
    #        ,col= adjustcolor( "darkgray", alpha.f = .2), border = adjustcolor( "darkgray", alpha.f = .2), cex = 1.75)  
    points((countydata_rnull$t_start-1 + nrow(ger_rnull) - nrow(countydata_rnull) ), countydata_rnull$Mean, pch=20, cex=1, col=adjustcolor("black", alpha.f=.8))
    
    # For the x-Axis
    upperdate <- as.Date(ger_rnull$date, "%Y-%m-%d") + (windowr0-1)
    upperdate <- gsub("2020-", "", upperdate)
    upperdate <- ifelse(substr(upperdate, 1,2) == "03"
                        , gsub("03-", "03/", upperdate)
                        , upperdate)
    ...
    axis(1
         , at = seq(1, nrow(ger_rnull), 1)
         , labels = weeks[(nrow(ger_rnull)-30):nrow(ger_rnull)]
         , las=2, tck=-.02
         , cex.axis = .8
         , col = "darkgray"
         , col.axis = "darkgray"
    ...
    axis(2
         , at = seq(-0.25, yaxpvalues[2], by = 0.2)
         , las=2, tck=-.02
         , cex.axis = .8
         , col = "darkgray"
         , col.axis = "darkgray"
    ...
    max_tick <- par("usr")[4]
    text(median(seq(1, nrow(ger_rnull), 1)), (par("usr")[4])
    ...
    #text(median(seq(1, nrow(ger_rnull), 1)), (max(c(max(ger_rnull$Quantile.0.975, na.rm = T), max(state_rnull$Quantile.0.975, na.rm = T), max(countydata_rnull$Quantile.0.975, na.rm = T))))
    #     , paste("Unter der Annahme eines durchschnittlichen seriellen Intervalls von",round(meansi),"Tagen \n und einer Standardabweichung von", sdsi, "Tagen \n mit R - EpiEstim 2.2-1."), pos = 1,  srt=0, xpd = TRUE, cex = .8,  col = "black")   
    mtext("Projekt DiVaCor.org | Grafik von Dr. Benjamin G. Engst & David M. Grundmanns"
          ,side=1, line=6, cex= .8, col = "darkgray", adj=-.225)
    
    legend("topright"
           , legend = c("Deutschlandweit", paste(unique(state_rnull$name)), paste(countyname))
