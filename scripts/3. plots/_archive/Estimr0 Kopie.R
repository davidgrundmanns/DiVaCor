...
#pdf(paste0("../../outcome/",staat,"_rnull.pdf"), width=10, height=5)
    ...
    plot(seq(1, nrow(ger), 1)
         , las = 1
         , type="n"
         , ylim=c(.5,(max(c(max(ger$Quantile.0.975, na.rm = T)
         ...
         , main= paste0("Gleitende durchschnittliche Reproduktionszahl von COVID-19 (Zeitfenster: ",windowr0," Tage)")
         , cex.lab = .9
         , cex.main = .9
         , frame.plot=F)
    ...
    # For the x-Axis
    upperdate <- as.Date(ger$date, "%Y-%m-%d") + windowr0
    ...
    #     , paste("Unter der Annahme eines durchschnittlichen seriellen Intervalls von",round(meansi),"Tagen \n und einer Standardabweichung von", sdsi, "Tagen \n mit R - EpiEstim 2.2-1."), pos = 1,  srt=0, xpd = TRUE, cex = .8,  col = "black")   
    text(par("usr")[1], par("usr")[3]*1.25
         , "Grafikcode von Dr. Benjamin G. Engst & David M. Grundmanns", pos = 4,  srt=0, xpd = TRUE, cex = .7,  col = "black")   
