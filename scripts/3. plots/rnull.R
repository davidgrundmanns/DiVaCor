...
windowr0 <- 4
# Auf wieviele Tage wird der moving average berechnet?
windowma <- 7
# Welches Bundesland?
...
# federal
points(ger$t_start-1, ger$mainfper, pch=18, cex=1, col="navyblue", type = "b") 
# state
#points((state$t_start-1 + nrow(ger) - nrow(state)), state$mainfper, pch=16, cex=1, col="dodgerblue3", type = "b")

# For the x-Axis
upperdate <- as.Date(ger$date, "%Y-%m-%d")
...
date <- as.Date(ger$date, "%Y-%m-%d") - (windowma-1)
date <- gsub("2020-", "", date)
...
mtext("Anteil an Neuinfektionen in der Gesamtbevölkerung in %"
...
# federal
points(ger$t_start-1, ger$mainfper, pch=18, cex=1, col=adjustcolor( "navyblue", alpha.f = .5), type = "b") 
# state
points((state$t_start-1 + nrow(ger) - nrow(state)), state$mainfper, pch=16, cex=1, col="dodgerblue3", type = "b") 
...
legend("topleft"
       ...
       , col = c("dodgerblue3","navyblue")
       , pt.cex = c(1.5,1.5)
       , text.col = c("dodgerblue3","navyblue")
       , cex = 1  # Schriftgröße
...
# first filter for counties in state (rki_statenuts)
  ...
  for(j in 1:length(tempcounties)){
    ...
    ger_rnull <- ger[(nrow(ger)-20):nrow(ger),]
    state_rnull <- state[(nrow(state)-20):nrow(state),]
    countydata_rnull <- countydata[(nrow(countydata)-20):nrow(countydata),]
    
    # let last digit be dividable by 0.2 without adding digits
    ylimvalues <- round(c(0,(max(c(max(ger_rnull$Quantile.0.975, na.rm = T)
                                   ,max(state_rnull$Quantile.0.975, na.rm = T)
                                   ,max(countydata_rnull$Quantile.0.975, na.rm = T),1.5)
    ))
    ))
    ...
    plot(seq(1, nrow(ger_rnull), 1)
         ...
         , type="n"
         , ylim=ylimvalues
         , xlim=c(nrow(ger_rnull)-20,nrow(ger_rnull))
         , yaxp=c(0, yaxpvalues[2], yaxpvalues[3])
         , xlab=""
         , xaxt="n" 
         , yaxt="n"
         , ylab="Durschnittliche Reproduktionszahl [R0]"
         , main= "Gleitende durchschnittliche Reproduktionszahl von COVID-19 (über 4 Tage mit 95% KI)"
         , cex.lab = .9
         , cex.main = .9
         , col.axis = "darkgray"
         , col.lab = "darkgray" 
    ...
    for(n in 1:nrow(state_rnull)){
      segments(n+0.2, state_rnull$Quantile.0.025[n], n+0.2, state_rnull$Quantile.0.975[n], lty = 1, lwd = 1.75, lend = 0, col ="dodgerblue3")
      points(n+0.2, state_rnull$Mean[n], pch=20, cex=1, col=adjustcolor( "dodgerblue3", alpha.f = .8))
      segments((n-0.2), ger_rnull$Quantile.0.025[n], (n-0.2), ger_rnull$Quantile.0.975[n], lty = 1, lwd = 1.75, lend = 0, col = "navyblue")
      points((n-0.2), ger_rnull$Mean[n], pch=20, cex=1, col=adjustcolor( "navyblue", alpha.f = .8)) 
      segments(n, countydata_rnull$Quantile.0.025[n], n, countydata_rnull$Quantile.0.975[n], lty = 1, lwd = 1.75, lend = 0, col = "deepskyblue")
      points(n, countydata_rnull$Mean[n], pch=20, cex=1, col=adjustcolor("deepskyblue", alpha.f=.8))
    }
   
    # For the x-Axis
    upperdate <- as.Date(ger_rnull$date, "%Y-%m-%d")
    upperdate <- gsub("2020-", "", upperdate)
    upperdate <- ifelse(substr(upperdate, 1,2) == "03"
                        , gsub("03-", "03/", upperdate)
                        , upperdate)
    ...
    date <- as.Date(ger_rnull$date, "%Y-%m-%d") - (windowr0-1)
    date <- gsub("2020-", "", date)
    date <- ifelse(substr(date, 1,2) == "03"
                   , gsub("03-", "03/", date)
                   , date)
    ...
    axis(1
         ...
         , labels = weeks[(nrow(ger_rnull)-20):nrow(ger_rnull)]
         , las=2, tck=-.02
         , cex.axis = .8
         , col = "darkgray"
         , col.axis = "darkgray"
...
#          ,side=1, line=6, cex= .8, col = "darkgray", adj=-.225)
    ...
    legend("topright"
           ...
           , pch = c(20, 20, 20) # Kästchen mit den vergebenen Farben
           , col = c("navyblue","dodgerblue3","deepskyblue")
           , pt.cex = c(1,1,1)
           , text.col = c("navyblue","dodgerblue3","deepskyblue")
           , cex = .8  # Schriftgröße
           , bg = NULL # Keine Hintergrungfarbe
           , bty = "n"
           , inset = 0.033
    ...
    # ------------------ #
      ...
      if(tempcounties[j] != "DE600"){
        ...
        # county plot
        points((countydata$t_start-1 + nrow(ger) - nrow(countydata)), countydata$mainfper, pch=15, cex=1, col="deepskyblue", type = "b")
        # state
        points((state$t_start-1 + nrow(ger) - nrow(state)), state$mainfper, pch=16, cex=1, col="dodgerblue3", alpha.f = .5, type = "b")
        # federal
        points(ger$t_start-1, ger$mainfper, pch=18, cex=1, col=adjustcolor( "navyblue", alpha.f = .5), type = "b") 
...
#              ,side=1, line=6, cex= .8, col = "darkgray", adj=-.225)
        ...
        legend("topleft"
               ...
               , pch = c(15, 16, 18) # Kästchen mit den vergebenen Farben
               , col = c("deepskyblue","dodgerblue3","navyblue")
               , pt.cex = c(1.2,1.2,1.2)
               , text.col = c("deepskyblue","dodgerblue3","navyblue")
               , cex = 1  # Schriftgröße
               , bg = NULL # Keine Hintergrungfarbe
               , bty = "n"
               , inset = 0.033
        ...
        plot(seq(1, nrow(ger), 1)
             ...
             , xlab=""
             , xaxt="n"
             , ylab="Anteil an Neuinfektionen in der Gesamtbevölkerung in %"
             , main= "Gleitender durchschnittlicher Anteil der Gesamtbevölkerungen mit Neuinfektionen von COVID-19 in % (pro Woche)"
             , cex.lab = .9
             , cex.main = .9
             , col.axis = "darkgray"
        ...
        points((state$t_start-1 + nrow(ger) - nrow(state)), state$mainfper, pch=15, cex=1, col="deepskyblue", type = "b")
        # state
        #points((state$t_start-1 + nrow(ger) - nrow(state)), state$mainfper, pch=16, cex=1, col=adjustcolor( "dodgerblue3", alpha.f = .5), type = "b")
        # federal
        points(ger$t_start-1, ger$mainfper, pch=18, cex=1, col=adjustcolor( "navyblue", alpha.f = .5), type = "b") 
...
#              ,side=1, line=6, cex= .8, col = "darkgray", adj=-.225)
        ...
        legend("topleft"
               ...
               , pch = c(15,18) # Kästchen mit den vergebenen Farben
               , col = c("dodgerblue3","navyblue")
               , pt.cex = c(1.2,1.2)
               , text.col = c("dodgerblue3","navyblue")
               , cex = 1  # Schriftgröße
